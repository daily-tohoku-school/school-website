...
const getAcceptLanguageLocale = async (acceptLanguage, manifest, routesManifest) => {
    var _a;
    if (routesManifest.i18n) {
        const defaultLocale = (_a = routesManifest.i18n.defaultLocale) === null || _a === void 0 ? void 0 : _a.toLowerCase();
...
const triggerStaticRegeneration = async (options) => {
    ...
    const { SQSClient, SendMessageCommand } = await Promise.resolve().then(function () { return require('./index-427401f4.js'); });
...
const s3StorePage = async (options) => {
    const { S3Client } = await Promise.resolve().then(function () { return require('./S3Client-411e0d0f.js'); });
    ...
    const { PutObjectCommand } = await Promise.resolve().then(function () { return require('./PutObjectCommand-4f3dd7ef.js'); });
    await Promise.all([
...
const handleOriginResponse = async ({ event, manifest, prerenderManifest, routesManifest }) => {
    ...
    if (fallbackRoute.isStatic) {
        ...
        const { GetObjectCommand } = await Promise.resolve().then(function () { return require('./GetObjectCommand-fd00e401.js'); });
